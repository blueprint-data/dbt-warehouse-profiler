{% macro bigquery__validate_source(schema, table, database=none, output_format='text') %}
...
{% macro bigquery__validate_dataset_sources(schema, database=none, output_format='text') %}
  ...
  {% if output_format == 'json' %}
    {{ log(tojson(result), info=True) }}
  {% else %}
    {{ log('=== Validating sources in dataset: ' + schema + ' ===', info=True) }}
    {{ log('', info=True) }}
    {{ log('', info=True) }}
